            ...
            for (let key in this.edges) {
                if (this.edges[key].from === item.id || this.edges[key].to === item.id) {
                    ...
                    if (_cid !== -1) {
                        ...
                        //console.log(this.nodes[nid].ip,_cid);
                        groups[_cid].number++;
                    } else {
                        markers[++cid] = indicates;
                        groups[cid] = { type: this.nodes[nid].type, number: 0 };
                    }
                }
            }
        });
        ...
        for (let i in groups) {
            const _options = {
                ...
                clusterNodeProperties: {
                    ...
                    image: CiscoVis.NextFont.createSVG(groups[i].type + 's'),
                    shape: 'image'
                },
